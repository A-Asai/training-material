...
> ### このチュートリアルの概要
>
> このチュートリアルでは [Unicycler tutorial]({{site.baseurl}}/topics/assembly/tutorials/unicycler-assembly/tutorial.html) で作成された新しいゲノムアセンブリを用いて始めます。これは *E. coli* C のアセンブリで、この種の他の全ての完全な遺伝子のアセンブリと比較します。
>
> 1. TOC
> {:toc}
>
...
*E. coli* は最も研究されている生物の1つです。何百もの完全なゲノムがあります（実際には、Genbank にある *E. coli* のアセンブリの総数は10,500を超えています）。ここでは全ての完全な *E. coli* のゲノムを一度にアップロードする方法を示します。

## Galaxy に完全な *E. coli* ゲノムを取得する


私たちの最初の目標は全ての完全な *E. coli* のゲノムと私たちのアセンブリを比較して密接に関連するゲノムを特定し興味深いゲノムの変化を見つけることです。これを行うためにはアセンブリを他の全てのゲノムとアラインメントする必要があります。そしてアラインメントを行うためにはまずはこれらの他のゲノムを全て取得する必要があります。 

[NCBI](https://www.ncbi.nlm.nih.gov/) は全ての完全な *E. coli* ゲノムを保存しているリソースです。具体的には、 [こちら](https://www.ncbi.nlm.nih.gov/genome/genomes/167) にあります。見たところ、このリストには500を超えるゲノムが含まれているので手動でアップロードすると手根管症候群になってしまうので、それはやめておきたいです。Galaxy には同様の種類の大量なデータをセットでアップロードして管理するために特化して設計されたいくつかの機能があります。以下の2つの **ハンズオン** セクションで Galaxy に全ての完全な *E. coli* ゲノムをインポートする方法を紹介します。

> ### {% icon hands_on %} ハンズオン: 全ての完全な *E. coli* ゲノムのリストを用意する
>
>[NCBI にある *E. coli* ゲノムのリスト](https://www.ncbi.nlm.nih.gov/genome/genomes/167) を新しいウィンドウで開き2つのブラウザウィンドウ（チュートリアルと今開いたもの）を並んで表示させます。次に以下のビデオの手順に従いましょう。
>
>
...
> ### {% icon hands_on %} ハンズオン: Fixing assembly
>
...
> ### {% icon hands_on %} ハンズオン: Running LASTZ
...
> ### {% icon hands_on %} ハンズオン: Combining collection into a single dataset
...
> ### {% icon hands_on %} ハンズオン: Selecting random subset of data
...
> ### {% icon hands_on %} ハンズオン: Graphing alignment data
...
> ### {% icon hands_on %} ハンズオン: Filtering data
...
>
>  NOTE: You need to select the full dataset; not the down-sampled one, but [the one generated by the collection collapsing operation](#hands_on-ハンズオン-combining-collection-into-a-single-dataset).
>
{: .hands_on}
...
> ### {% icon hands_on %} ハンズオン: Aggregating the data
...
> ### {% icon hands_on %} ハンズオン: Graphing aggregated data
...
> ### {% icon hands_on %} ハンズオン: Extracting into about best hits
...
> ### {% icon hands_on %} ハンズオン: Uploading sequences and annotations
...
> ### {% icon hands_on %} ハンズオン: Aligning again
>
> 1. **LASTZ** {% icon tool %} with the following parameters:
>   - *"Select TARGET sequence(s) to align against"*: `from your history`
...
> ### {% icon hands_on %} ハンズオン: Creating a single FASTA dataset with all genomes
>
> 1. **Collapse Collection** {% icon tool %} with the following parameters:
...
> ### {% icon hands_on %} ハンズオン: Cleaning sequence names
...
> ### {% icon hands_on %} ハンズオン: "Grepping" FASTA headers
...
> ### {% icon hands_on %} ハンズオン: Concatenate FASTA files
...
> ### {% icon hands_on %} ハンズオン: Starting a custom IGV browser
...
One immediate problem is `%` character in column 12 (alignment identity). We need to remove it. For this we will use **SED** tool that should be familiar to us from [previous ハンズオン exercises](#hands_on-ハンズオン-cleaning-sequence-names):

> ### {% icon hands_on %} ハンズオン: Removing `%` character from LASTZ output
>
> 1. **Text transformation with sed** {% icon tool %} with the following parameters:
...
> ### {% icon hands_on %} ハンズオン: Collapsing all alignment info into a single dataset
> 1. **Collapse Collection** {% icon tool %} with the following parameters:
...
Alignments are regions of high similarity between two sequences. Therefore each alignment block has two sets of coordinates associated with it: start/end in the first sequences (target) and start/end in the second sequence (query). But BED only has one set of coordinates. Thus we can create two BEDs: one using coordinates from the target and the other one from query. The first file will depict alignment data from the standpoint of target sequences `CP020543.1`, `CP024090.1`, `LT906474.1` and the second from the standpoint of query - our own assembly [we called](#hands_on-ハンズオン-fixing-assembly) `Ecoli_C`. In the first BED, column 1 will contain names of targets (`CP020543.1`, `CP024090.1`, and `LT906474.1`). In the second BED, column 1 will contain name of our assembly: `Ecoli_C`. To create the first BED we will cut six columns from the dataset produced at the last step. Specifically, to produce the target BED we will cut columns 2, 4, 5, 14, 12, and 8. To produce the query BED columns 7,9,10,14,12,8 will be cut.

> ### {% icon warning %} There are multiple **CUT** tools!
> The ハンズオン box below uses **Cut** tool. Beware that some Galaxy instances contain multiple **Cut** tools. The one that is used below is called **Cut columns from a table** while the other one, which we will NOT use is called **Cut columns from a table (cut)**. It is a small difference, but the tools are different.
{: .warning-box}

> ### {% icon hands_on %} ハンズオン: Creating target BED
...
> ### {% icon hands_on %} ハンズオン: Creating query BED
> 1. **Cut columns from a table** {% icon tool %} with the following parameters
...
> ### {% icon hands_on %} ハンズオン: Merging Target and Query BEDs
>
...
> ### {% icon hands_on %} ハンズオン: Changing dataset type
>
...
Now we are ready to display these data in the browser (make sure the browser we've created [above](#hands_on-ハンズオン-starting-a-custom-igv-browser) is open):

> ### {% icon hands_on %} ハンズオン: Display alignments in the browser
>
...
> ### {% icon hands_on %} ハンズオン: Retain CDS rows in annotation datasets
> 1. **Select lines that match an expression** {% icon tool %} with the following parameters:
...
> ### {% icon hands_on %} ハンズオン: Creating a dummy score column
>
...
> ### {% icon hands_on %} ハンズオン: Cutting columns form annotation data
>
> 1. **Cut columns from a table** {% icon tool %} with the following parameters:
...
However, before we convert coordinates of alignments into their complements we need to prepare a so called *genome file*, which is a list of "chromosomes" and their lengths in our [hybrid genome](#hands_on-ハンズオン-concatenate-fasta-files):

> ### {% icon hands_on %} ハンズオン: Creating a genome file
>
> 1. **Compute sequence length** {% icon tool %} with the following parameters:
>   - *"Compute length for these sequences"*: the FASTA dataset we generated [concatenated "hybrid" genome](#hands_on-ハンズオン-concatenate-fasta-files)
{: .hands_on}
...
> ### {% icon hands_on %} ハンズオン: Sorting genome file
>
...
Next we need to go back to the BED file containing [alignment data](#hands_on-ハンズオン-changing-dataset-type) and sort it as well:

> ### {% icon hands_on %} ハンズオン: Sorting BED file
> 1. **SortBED order the intervals** {% icon tool %} with the following parameters
>   -  *"Sort the following BED file"*: our [alignment BED](#hands_on-ハンズオン-changing-dataset-type)
...
>  - *"Genome file"*: sorted genome file we've generated [two steps ago](#hands_on-ハンズオン-sorting-genome-file)
{: .hands_on}
...
At this point we have two BEDs: one just created and the other containing [gene annotations](#hands_on-ハンズオン-collapsing-annotations-into-a-single-bed-dataset). We can simply intersect the two:
...
> ### {% icon hands_on %} ハンズオン: Filtering BED on interval length
>
> 1. **Filter data on any column using simple expressions** {% icon tool %} with the following parameters:
...
> ### {% icon hands_on %} ハンズオン: Restricting list of deleted regions to the *common* deletion
>
> 1. **Filter data on any column using simple expressions** {% icon tool %} with the following parameters:
...
Now we are ready to intersect these regions with gene coordinates we formatted [earlier](#hands_on-ハンズオン-collapsing-annotations-into-a-single-bed-dataset):

> ### {% icon hands_on %} ハンズオン: Finding genes deleted in our assembly
>
...
>  - *"File(s) B to intersect with A"*: gene annotations in [BED format](#hands_on-ハンズオン-collapsing-annotations-into-a-single-bed-dataset)
...
> ### {% icon hands_on %} ハンズオン: Converting a list of essential genes to TAB-delimited format
>
...
The two truly important columns here are 1 (gene name) and 4 (is gene essential?). Let's join the results of the [intersection](#hands_on-ハンズオン-finding-genes-deleted-in-our-assembly) with this list:

> ### {% icon hands_on %} ハンズオン: Are there essential genes?
>
> 1. **Join two Datasets side by side on a specified field** {% icon tool %} with the following parameters:
>   - *"Join"*: the results of the [intersect operation](#hands_on-ハンズオン-finding-genes-deleted-in-our-assembly)(`Intersect intervals on data...`)
