...
このチュートリアルでは *E. coli*  [C-1](http://cgsc2.biology.yale.edu/Strain.php?ID=8232) 株のゲノムを集めてアノテーションを行います。 この株はバクテリオファージを含む実験的な進化研究において日常的に用いられています。例えば、Holly Wichman と Jim Bull による古典的な研究 ([Bull et al. 1997](https://www.ncbi.nlm.nih.gov/pubmed/9409816), [Bull & Wichman 1998](https://www.ncbi.nlm.nih.gov/pubmed/9767038), [Wichman et al. 1999](https://www.ncbi.nlm.nih.gov/pubmed/10411508)) ではこの株とバクテリオファージ phiX174を用いて実施されています。

ゲノム配列を決定するために [Yale E. coli Stock Center](http://cgsc2.biology.yale.edu/) からこの株を手に入れました。このストックセンターは細胞をろ紙のディスクに注入して送ってきました。送られてきたディスクはLB寒天プレートの中央に置きました。その後シングルコロニーを採取してLB液体培地に再懸濁し、一晩増殖させて、ゲノムDNAを単離しました。そしてDNAを2つの方法を用いて配列を決定しました。高いカバレッジや高精度のデータを得るために、Illumina 社の miSEQ を用いて250 bp のペアエンドリードを生成しました。長いリード長を生成するためには Oxford Nanopore 社の MinIon machine を用いた。

私たちの目標は *E. coli* C-1 の全ゲノムを再構成しアノテーションを行うことです。このチュートリアルではほぼ完璧なアセンブリを手がけるために、多くの短く高精度のリードと、長くエラーの起こりやすいリードを組み合わせます。
...
このチュートリアルでは2種類のインプットデータを用いてゲノムアセンブリを行います: (1) Illumina 社の 250 bp のペアエンドリードと (2) Oxford Nanopore 社のリード。
...
> ### {% icon hands_on %} ハンズオン: Load data into History
>
...
> ### {% icon hands_on %} ハンズオン: Quality Control
>
...
> ### {% icon hands_on %} ハンズオン: Unicycler Assembly
>
...
> ### {% icon hands_on %} ハンズオン: Assembly Quality
>
...
> ### {% icon hands_on %} ハンズオン: Annotation
>
...
> ### {% icon hands_on %} ハンズオン: Visualization in IGV
>
>
