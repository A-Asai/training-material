...
この実戦での目的はGalaxyのユーザーインターフェースに慣れることです。ここではデータのインポートや、ツールの実行、ヒストリーの操作、ワークフローの作成、作業の共有といった基本的なタスクのやり方を教えます。

> ### アジェンダ
>
> このチュートリアルでは、以下のものを扱います:
>
> 1. TOC
> {:toc}
>
...
> ### {% icon question %} 問題
> *どのコーディングエキソンがヒト22番染色体上で一塩基多型（SNP）の数が最も多いでしょうか？*
{: .question}

あなたは「ワオ！これは簡単な問題ですね...私はどこでデータを見つけるか知っていて、[UCSC Genome Browser](https://genome.ucsc.edu/)で見つかりますが、実際にどうやって」You are thinking "Wow! This is a simple question... I know where to find the data, at the [UCSC Genome Browser](https://genome.ucsc.edu/), but how do I actually compute this?" There is really a straightforward way of answering this question and it is called **Galaxy**. So let's try it...
...
> ### {% icon hands_on %} ハンズオン: Create history
>
...
> ### {% icon hands_on %} ハンズオン: Data upload from UCSC
>
...
> ### {% icon hands_on %} ハンズオン: SNP information
...
> ### {% icon hands_on %} ハンズオン: Finding Exons
>
...
> ### {% icon hands_on %} ハンズオン: Counting SNPs
>
...
> ### {% icon hands_on %} ハンズオン: Select first
>
...
> ### {% icon hands_on %} ハンズオン: Compare two Datasets
>
...
> ### {% icon hands_on %} ハンズオン: UCSC genome browser
>
...
> ### {% icon hands_on %} ハンズオン: Extract workflow
>
...
> ### {% icon hands_on %} ハンズオン: Run workflow
>
...
> ### {% icon hands_on %} ハンズオン: Share history and workflow
>
